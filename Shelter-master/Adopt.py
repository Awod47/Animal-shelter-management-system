...
def openAdopt():
    ...
    btn4 = Button(root_adopt,text="Adoption Details",bg='black', fg='white', command=viewDetails)
...
def viewDetails():
    ...
    def lookup_records():
        ...
        def sqSearchAdopt(key):
            cur.execute("SELECT * FROM DETAILS WHERE CUST_ID LIKE ? OR CUST_NAME LIKE ? OR PHONE LIKE ? OR ADDRESS LIKE ? OR AN_ID LIKE ? OR AN_NAME LIKE ? OR BREED LIKE ? OR SPECIES LIKE ? OR AD_DATE LIKE ?;",(key,key,key,key,key,key,key,key,key))
            adlist=cur.fetchall()
            return adlist 
        ...
        def search_records():
            ...
            i=0
            for r in records:
                tree.insert('', i, text = "", values = (r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], r[8]))
                i+= 1
    ...
    cur.execute("CREATE VIEW IF NOT EXISTS 'DETAILS' AS SELECT ANIMAL.AN_ID, ANIMAL.AN_NAME, ANIMAL.BREED, ANIMAL.SPECIES, CUSTOMER.CUST_ID, CUSTOMER.CUST_NAME, CUSTOMER.PHONE, CUSTOMER.ADDRESS, ADOPT.AD_DATE FROM ANIMAL JOIN ADOPT ON ADOPT.AN_ID = ANIMAL.AN_ID JOIN CUSTOMER ON ADOPT.CUST_ID = CUSTOMER.CUST_ID;")
    ...
    tree.column("ad_date", width=150, minwidth=50, anchor=CENTER)
